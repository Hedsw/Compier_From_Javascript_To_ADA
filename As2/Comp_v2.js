...
var idt_Error = (/CONSTANT|MODULE|BEGIN|PROCEDURE|IS|IF|THEN|ELSE|ELSIF|WHILE|LOOP|FLOAT|INTEGER|CHAR|CHAR|GET|PUT|END|--|[;]|[:]|[()]|[=]|[<]|[>]|[<=]|[>=]|[*]|[/]|rem|and|and|or/gi);
var comments = (/-- */);
var rel_ops = /[;]|[:]|[()]|[=]|[<]|[>]|[<=]|[>=]/gi;
...
//begin end check
    ...
    for(var i = 0; i< lines.length; i++) {
      if(lines[i].match(procedure_match) != null) {
        ...
        idt_check = tmp_line[1];
         if(idt_check.match(idt_Error) !=null) {
            console.log("Error Reserved word is in idt position");
         }
      }
  ...
  if(procedure_count == 2) {
     if(lines[0].match(procedure_match) != null) {
        ...
        if(idt_check2.match(idt_Error) !=null) {
            console.log("Error Reserved word is in idt position");
         }
      }
    for(var i = 0; i< lines.length; i++) {
